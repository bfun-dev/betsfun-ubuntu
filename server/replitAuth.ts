...
export function getSession() {
  ...
  return session({
    ...
    cookie: {
      httpOnly: true,
      secure: false, // Allow cookies over HTTP for localhost production builds
      maxAge: sessionTtl,
      sameSite: 'lax',
    },
  });
...
export const isAuthenticated: RequestHandler = async (req, res, next) => {
  ...
  console.log("Auth check - Session data:", {
    ...
    hasClaims: !!session?.user?.claims,
    userId: session?.user?.claims?.sub,
  ...
  if (session?.user?.claims?.sub) {
    ...
    console.log("Auth success - Web3Auth session found for user:", session.user.claims.sub);
    return next();
  }
