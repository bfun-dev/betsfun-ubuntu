...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.use((req, res, next) => {
    ...
    // Allow localhost, 127.0.0.1, and same-origin requests
  ...
  // AI Insights for market analysis - Premium feature
  app.post("/api/ai-insights", isAuthenticated, async (req, res) => {
    try {
      ...
      // Check if user has AI Insights subscription or Ultimate Bundle
...
Keep responses concise and actionable.`;
  ...
  app.post('/api/auth/web3auth', async (req, res) => {
    try {
      console.log('🔐 Web3Auth authentication request received');
      ...
      if (existingUser) {
        ...
        console.log('📤 Sending response for existing user:', existingUser.id);
  ...
  // Comprehensive logout endpoint with Web3Auth session termination
  app.post('/api/logout', (req, res) => {
    ...
    const clearAllCookies = () => {
      // Get all cookies from the request to clear them properly
      ...
      const cookiesToClear = [
        ...
        'sid',
        'express.sid',
        ...allCookieNames.filter(name => !name.includes(':')) // Exclude cookies with colons
      ];
      
      // Remove duplicates and filter valid cookie names
      const uniqueCookies = [...new Set(cookiesToClear)];
      
      uniqueCookies.forEach(cookieName => {
        // Only clear cookies with valid names
        if (cookieName && typeof cookieName === 'string' && cookieName.trim() && cookieName.length > 0) {
          try {
            // Clear with minimal options first
            res.clearCookie(cookieName);
            
            // Clear with path
            res.clearCookie(cookieName, { path: '/' });
            
            // Clear with secure options if needed
            res.clearCookie(cookieName, {
              path: '/',
              httpOnly: true,
              secure: false,
    ...
    if (session) {
      ...
      // Save the logout flag, then destroy
      session.save((saveErr) => {
        ...
        session.destroy((err: any) => {
          ...
          // Clear all cookies regardless of session destruction success
          clearAllCookies();
  ...
  // Helper function to get wallet info for a user
  async function getUserWalletInfo(userId: string) {
    const user = await storage.getUser(userId);
    if (!user) {
    ...
    const [ethData, solData] = await Promise.all([
      ethereumAddress && ethereumAddress !== 'Not connected' && ethereumAddress.length === 42
        ? fetchEthereumTokens(ethereumAddress).catch(() => ({ eth: '0', tokens: [] }))
        : Promise.resolve({ eth: '0', tokens: [] }),
      solanaAddress
        ? fetchSolanaTokens(solanaAddress).catch(() => ({ sol: '0', tokens: [] }))
        : Promise.resolve({ sol: '0', tokens: [] })
    ]);
  ...
  app.post('/api/wallet/withdraw', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const currentBalance = type === 'eth' 
        ? parseFloat(walletInfo.ethereum?.balance || '0')
        : parseFloat(walletInfo.solana?.balance || '0');

      if (withdrawAmount > currentBalance) {
        return res.status(400).json({ error: "Insufficient balance for withdrawal" });
      }

      // Simulate withdrawal process (in a real implementation, this would interact with blockchain)
      // For now, we'll update the user's platform balance
      
      // Convert crypto amount to USD for platform balance adjustment
      let usdValue = 0;
      if (type === 'eth') {
        usdValue = withdrawAmount * 3500; // ETH price approximation
      } else if (type === 'sol') {
        usdValue = withdrawAmount * 140; // SOL price approximation
      }

      // Get user and deduct from platform balance
      const user = await storage.getUser(userId);
  ...
  // Download deployment package endpoint
  app.get('/api/download/namecheap-deploy', (req, res) => {
    try {
  ...
  // Public subscription data endpoint for user-facing components
  app.get('/api/subscriptions', async (req, res) => {
    try {
      const { subscriptionStore } = await import('./subscription-store');
      const plans = await subscriptionStore.getPlans();
      const bundle = await subscriptionStore.getBundle();
  ...
  app.patch('/api/admin/subscriptions/:planId', isAdminAuthenticated, async (req, res) => {
    try {
      ...
      const { subscriptionStore } = await import('./subscription-store');
      const result = await subscriptionStore.updatePlan(planId, updatedPlan);
  ...
  app.patch('/api/admin/subscriptions/bundle', isAdminAuthenticated, async (req, res) => {
    try {
      ...
      const { subscriptionStore } = await import('./subscription-store');
      const result = await subscriptionStore.updateBundle(bundleData);
  ...
  app.post('/api/subscriptions/purchase', isAuthenticated, async (req, res) => {
    try {
      ...
      // Get plan details from subscription store
      const { subscriptionStore } = await import('./subscription-store');
      ...
      if (planId === "ultimate-bundle") {
        ...
        // Check if user already has both individual plans (which would make bundle redundant)
        const plans = await subscriptionStore.getPlans();
        ...
        // Purchase bundle (this will need special handling in storage)
        ...
        // Handle individual plan purchase
        const plans = await subscriptionStore.getPlans();
  ...
  app.get('/api/user/subscriptions/:planId/status', isAuthenticated, async (req, res) => {
    try {
      const userId = req.user?.claims?.sub;
      const { planId } = req.params;
