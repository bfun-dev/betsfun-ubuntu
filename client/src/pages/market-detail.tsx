...
import { ExternalLink, Sparkles, BarChart3, AlertCircle, Crown, Star } from "lucide-react";
...
export default function MarketDetail() {
  ...
  // Check if user has AI Insights subscription
  useEffect(() => {
    const checkSubscription = async () => {
      ...
      try {
        // Check AI Insights subscription
        const aiResponse = await fetch('/api/user/subscriptions/ai-insights/status');
        const aiData = await aiResponse.json();
        
        // Check Ultimate Bundle subscription
        const bundleResponse = await fetch('/api/user/subscriptions/ultimate-bundle/status');
        const bundleData = await bundleResponse.json();
        
        // User has access if they have either AI Insights or Ultimate Bundle
        const hasAccess = aiData.hasActiveSubscription || bundleData.hasActiveSubscription;
        setHasAiInsights(hasAccess);
      } catch (error) {
  ...
  const generateAIInsights = async () => {
    ...
    // Check if user has subscription before making API call
    if (!hasAiInsights) {
      // Show golden upgrade prompt for non-subscribers
      setAiInsights("upgrade_required");
    ...
    try {
      ...
      setAiInsights({
        ...
        sentimentAnalysis: "Unable to generate insights at this time",
        ...
        <Card className={`${isMobile ? 'mb-3' : 'mb-6'}`}>
          {!aiInsightsExpanded ? (
            <Button
              ...
              className={`w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white border-0 ${isMobile ? 'h-9 text-xs px-3' : 'h-12'} disabled:opacity-50`}
            >
              <Sparkles className={`${isMobile ? 'h-3.5 w-3.5' : 'h-5 w-5'} mr-2 ${loadingInsights ? 'animate-spin' : ''}`} />
            ...
            <div className={`bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-950/20 dark:to-pink-950/20 border-purple-200 dark:border-purple-800 ${isMobile ? 'p-3' : 'p-4'}`}>
              <div className={`flex items-start ${isMobile ? 'gap-2' : 'gap-3'}`}>
                ...
                <div className={`flex-1 ${isMobile ? 'space-y-1' : 'space-y-2'}`}>
                  ...
                  {aiInsights === "upgrade_required" ? (
                    /* Golden Upgrade Prompt */
                    <div className="mb-3">
                      <div className={`bg-gradient-to-r from-yellow-50 to-amber-50 dark:from-yellow-950/20 dark:to-amber-950/20 border-2 border-yellow-300 dark:border-yellow-700 rounded-lg ${isMobile ? 'p-3' : 'p-4'}`}>
                        <div className={`flex items-start ${isMobile ? 'gap-2' : 'gap-3'}`}>
                          <div className="w-12 h-12 bg-gradient-to-r from-yellow-500 to-amber-500 rounded-full flex items-center justify-center flex-shrink-0">
                            <Crown className="h-6 w-6 text-white" />
                          </div>
                          <div className={`flex-1 ${isMobile ? 'space-y-2' : 'space-y-3'}`}>
                            <div className="flex items-center gap-2">
                              <h3 className={`font-bold text-yellow-900 dark:text-yellow-100 ${isMobile ? 'text-sm' : 'text-lg'}`}>
                                Unlock AI-Powered Insights
                              </h3>
                              <Star className="h-4 w-4 text-yellow-600 dark:text-yellow-400" />
                            </div>
                            <p className={`text-yellow-800 dark:text-yellow-200 ${isMobile ? 'text-xs' : 'text-sm'}`}>
                              Get intelligent market analysis, sentiment predictions, and risk assessments to <strong>maximize your win probability</strong>. 
                              Our AI analyzes thousands of data points to give you the edge.
                            </p>
                            <div className={`grid grid-cols-1 md:grid-cols-3 gap-2 ${isMobile ? 'mt-2' : 'mt-3'}`}>
                              <div className="flex items-center gap-2">
                                <div className="w-2 h-2 bg-yellow-500 rounded-full"></div>
                                <span className={`text-yellow-700 dark:text-yellow-300 ${isMobile ? 'text-xs' : 'text-sm'}`}>Sentiment Analysis</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <div className="w-2 h-2 bg-yellow-500 rounded-full"></div>
                                <span className={`text-yellow-700 dark:text-yellow-300 ${isMobile ? 'text-xs' : 'text-sm'}`}>Trend Predictions</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <div className="w-2 h-2 bg-yellow-500 rounded-full"></div>
                                <span className={`text-yellow-700 dark:text-yellow-300 ${isMobile ? 'text-xs' : 'text-sm'}`}>Risk Assessment</span>
                              </div>
                            </div>
                            <Button
                              className={`w-full bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-600 hover:to-amber-600 text-white font-semibold shadow-lg ${isMobile ? 'h-9 text-xs' : 'h-11'}`}
                              onClick={() => window.location.href = '/profile'}
                            >
                              <Crown className={`${isMobile ? 'h-3.5 w-3.5' : 'h-4 w-4'} mr-2`} />
                              Buy AI Insights Package - $4.99/month
                            </Button>
                          </div>
                        </div>
                      </div>
                    ...
                    <>
                      <div className={`mb-3 ${isMobile ? 'space-y-2' : 'space-y-3'}`}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div className="p-3 bg-white/50 dark:bg-black/20 rounded-lg border border-purple-200/50 dark:border-purple-700/50">
                          <h4 className={`font-medium text-purple-900 dark:text-purple-100 ${isMobile ? 'text-xs' : 'text-sm'}`}>
                            Sentiment Analysis
                          </h4>
                          <div className="flex items-center gap-2 mt-1">
                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                              aiInsights.sentiment === 'positive' ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300' :
                              aiInsights.sentiment === 'negative' ? 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-300' :
                              'bg-gray-100 text-gray-800 dark:bg-gray-900/20 dark:text-gray-300'
                            }`}>
                              {aiInsights.sentiment}
                            </span>
                            <span className={`text-purple-600 dark:text-purple-400 ${isMobile ? 'text-xs' : 'text-sm'}`}>
                              {Math.round(aiInsights.sentimentConfidence * 100)}%
                            </span>
                          </div>
                          <p className={`text-purple-700 dark:text-purple-300 ${isMobile ? 'text-xs' : 'text-xs'} mt-1`}>
                        ...
                        <div className="p-3 bg-white/50 dark:bg-black/20 rounded-lg border border-purple-200/50 dark:border-purple-700/50">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                              aiInsights.trendDirection === 'bullish' ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300' :
                              aiInsights.trendDirection === 'bearish' ? 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-300' :
                              'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300'
                            }`}>
                              {aiInsights.trendDirection}
                            </span>
                          </div>
                          <p className={`text-purple-700 dark:text-purple-300 ${isMobile ? 'text-xs' : 'text-xs'} mt-1`}>
                        ...
                        <div className="p-3 bg-white/50 dark:bg-black/20 rounded-lg border border-purple-200/50 dark:border-purple-700/50">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                              aiInsights.riskAssessment === 'low' ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300' :
                              aiInsights.riskAssessment === 'high' ? 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-300' :
                              'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300'
                            }`}>
