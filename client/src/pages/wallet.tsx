...
import { Copy, ExternalLink, Eye, EyeOff, Wallet, ArrowUpRight, ArrowDownLeft, RefreshCw, Shield, Globe, Key, Plus, Minus, ArrowLeftRight, CreditCard, Zap } from "lucide-react";
...
import Web3AuthWalletPreview from "@/components/web3auth-wallet-preview";
...
export default function WalletPage() {
  ...
  const withdrawMutation = useMutation({
    mutationFn: async ({ amount, address, type }: { amount: string; address: string; type: 'eth' | 'sol' }) => {
      return await apiRequest('POST', '/api/wallet/withdraw', { amount, address, type });
    },
    onSuccess: () => {
      toast({
        title: "Withdrawal Successful",
        description: "Funds have been transferred to your wallet address",
      });
  ...
  const handleWithdraw = () => {
    ...
    const isValidAddress = transferType === 'eth' 
      ? /^0x[a-fA-F0-9]{40}$/.test(withdrawAddress)
      : /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(withdrawAddress);

    if (!isValidAddress) {
      toast({
        title: "Invalid Address",
        description: `Please enter a valid ${transferType.toUpperCase()} wallet address`,
        variant: "destructive",
      });
      return;
    }
  ...
  const formatCryptoBalance = (balance: string | number, decimals: number = 8) => {
                        ...
                        <DialogContent className="max-w-md">
                          <DialogHeader className="text-center">
                            <DialogTitle className="text-xl font-semibold">
                              Withdraw {transferType.toUpperCase()}
                            </DialogTitle>
                            <DialogDescription className="text-sm text-muted-foreground">
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="p-3 bg-muted/50 rounded-lg">
                              ...
                              <div className="text-lg font-semibold">
                                {transferType === 'eth' 
                                  ? `${formatCryptoBalance(walletInfo?.ethereum?.balance || '0')} ETH`
                                  : `${formatCryptoBalance(walletInfo?.solana?.balance || '0')} SOL`
                                }
                              </div>
                              <div className="text-xs text-muted-foreground">
                                ${transferType === 'eth' 
                                  ? (parseFloat(walletInfo?.ethereum?.balance || '0') * 3500).toFixed(2)
                                  : (parseFloat(walletInfo?.solana?.balance || '0') * 140).toFixed(2)
                                }
                              </div>
                            </div>
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="relative">
                                <Input
                                  ...
                                  type="number"
                                  step="0.000001"
                                  placeholder={`Enter ${transferType.toUpperCase()} amount`}
                              ...
                              <Button
                                ...
                                className="text-xs"
                                onClick={() => {
                                  const balance = transferType === 'eth' 
                                    ? walletInfo?.ethereum?.balance || '0'
                                    : walletInfo?.solana?.balance || '0';
                            ...
                            <div className="space-y-2">
                              <Label htmlFor="withdraw-address">
                                Recipient {transferType.toUpperCase()} Address
                              </Label>
                              <Input
                                id="withdraw-address"
                                type="text"
                            ...
                            <div className="p-3 bg-yellow-50 dark:bg-yellow-950/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                              <div className="flex items-start gap-2">
                                <div className="w-4 h-4 rounded-full bg-yellow-500 flex items-center justify-center text-white text-xs font-bold mt-0.5">!</div>
                                <div className="text-xs">
                                  <div className="font-medium text-yellow-800 dark:text-yellow-200">Network Fee Notice</div>
                                  <div className="text-yellow-700 dark:text-yellow-300 mt-1">
                                    Network fees will be deducted from the withdrawal amount. 
                                    Estimated fee: {transferType === 'eth' ? '~$5-15' : '~$0.01-0.05'}
                                  </div>
                                </div>
                              </div>
                            </div>
                      ...
                      {walletInfo.ethereum.balance && parseFloat(walletInfo.ethereum.balance) > 0 ? (
                        <div className="flex items-center justify-between py-1">
                          ...
                          <div className="flex flex-col items-end">
                            <span className="text-sm font-mono">{formatCryptoBalance(walletInfo.ethereum.balance)}</span>
                            {tokenPrices && (
                      ...
                      <div className="mt-3 pt-3 border-t border-border/50">
                        ...
                        {walletInfo.solana.balance && parseFloat(walletInfo.solana.balance) > 0 ? (
                          <div className="flex items-center justify-between py-1">
                            ...
                            <div className="flex flex-col items-end">
                              <span className="text-sm font-mono">{formatCryptoBalance(walletInfo.solana.balance)}</span>
                              {tokenPrices && (
                                <span className="text-xs text-muted-foreground">
