...
__export(schema_exports, {
  ...
  insertSubscriptionSettingsSchema: () => insertSubscriptionSettingsSchema,
...
var sessions, users, categories, markets, bets, badges, userBadges, walletData, subscriptionPlans, subscriptionSettings, userSubscriptions, usersRelations, categoriesRelations, marketsRelations, betsRelations, badgesRelations, userBadgesRelations, userSubscriptionsRelations, insertCategorySchema, insertMarketSchema, insertBetSchema, insertBadgeSchema, insertUserBadgeSchema, insertSubscriptionPlanSchema, insertSubscriptionSettingsSchema, insertUserSubscriptionSchema;
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    walletData = pgTable("wallet_data", {
      id: serial("id").primaryKey(),
    ...
    subscriptionSettings = pgTable("subscription_settings", {
      id: serial("id").primaryKey(),
    ...
    marketsRelations = relations(markets, ({ one, many }) => ({
    ...
    betsRelations = relations(bets, ({ one }) => ({
      user: one(users, {
        fields: [bets.userId],
        references: [users.id]
      }),
    ...
    insertMarketSchema = createInsertSchema(markets).omit({
      id: true,
      totalVolume: true,
      yesPrice: true,
      noPrice: true,
    ...
    insertSubscriptionSettingsSchema = createInsertSchema(subscriptionSettings).omit({
      id: true,
      updatedAt: true
    });
...
// server/subscription-store.ts
var subscription_store_exports = {};
__export(subscription_store_exports, {
...
async function initializeDatabase() {
  try {
    ...
    const existingBundle = await db.select().from(subscriptionSettings).where(eq2(subscriptionSettings.key, "bundle"));
...
var defaultSubscriptionPlans, defaultBundleSettings, subscriptionStore;
var init_subscription_store = __esm({
  "server/subscription-store.ts"() {
    ...
    subscriptionStore = {
      ...
      async getBundle() {
        try {
          const [result] = await db.select().from(subscriptionSettings).where(eq2(subscriptionSettings.key, "bundle"));
          return result ? result.value : defaultBundleSettings;
        } catch (error) {
      ...
      async updatePlan(planId, updatedPlan) {
        try {
          const updateData = { ...updatedPlan };
          if (updatedPlan.price !== void 0) {
            updateData.price = updatedPlan.price.toString();
          }
          updateData.updatedAt = /* @__PURE__ */ new Date();
          const [result] = await db.update(subscriptionPlans).set(updateData).where(eq2(subscriptionPlans.id, planId)).returning();
          return result ? {
            id: result.id,
            name: result.name,
      ...
      async updateBundle(updatedBundle) {
        try {
          ...
          const [result] = await db.update(subscriptionSettings).set({
            value: newBundle,
            updatedAt: /* @__PURE__ */ new Date()
          }).where(eq2(subscriptionSettings.key, "bundle")).returning();
          return result ? result.value : defaultBundleSettings;
        } catch (error) {
...
var DatabaseStorage = class {
  ...
  async purchaseSubscription(userId, planId, price) {
    return await db.transaction(async (tx) => {
      ...
      const newBalance = (currentBalance - subscriptionPrice).toFixed(2);
  ...
  async getUserSubscriptions(userId) {
    return await db.select().from(userSubscriptions).where(and(eq(userSubscriptions.userId, userId), eq(userSubscriptions.isActive, true)));
  }
  async hasActiveSubscription(userId, planId) {
    const [subscription] = await db.select().from(userSubscriptions).where(
      and(
        eq(userSubscriptions.userId, userId),
        eq(userSubscriptions.planId, planId),
        eq(userSubscriptions.isActive, true),
        gt(userSubscriptions.expiresAt, /* @__PURE__ */ new Date())
      )
    );
    return !!subscription;
  }
...
function getSession() {
  ...
  return session({
    ...
    cookie: {
      httpOnly: true,
      secure: false,
      // Allow cookies over HTTP for localhost production builds
      maxAge: sessionTtl,
      sameSite: "lax"
    }
  });
...
var isAuthenticated = async (req, res, next) => {
  ...
  console.log("Auth check - Session data:", {
    ...
    hasClaims: !!session2?.user?.claims,
    userId: session2?.user?.claims?.sub,
    loggedOut: session2?.loggedOut
  });
  if (session2?.loggedOut === true) {
    console.log("Auth blocked - User explicitly logged out");
    return res.status(401).json({ message: "User logged out" });
  }
  if (session2?.user?.claims?.sub) {
    req.user = session2.user;
    console.log("Auth success - Web3Auth session found for user:", session2.user.claims.sub);
    return next();
  }
  const user = req.user;
  console.log("Auth check - OAuth user:", {
  ...
  if (now <= user.expires_at) {
    console.log("Auth success - OAuth token valid");
    return next();
  }
...
async function registerRoutes(app2) {
  ...
  app2.post("/api/ai-insights", isAuthenticated, async (req, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
      }
      const hasAIInsights = await storage.hasActiveSubscription(userId, "ai-insights");
      const hasBundle = await storage.hasActiveSubscription(userId, "ultimate-bundle");
      if (!hasAIInsights && !hasBundle) {
        return res.status(403).json({
          message: "AI Insights requires premium subscription",
          requiresUpgrade: true,
          subscriptionTier: "ai-insights"
        });
      }
      const { marketTitle, marketDescription } = req.body;
      if (!marketTitle) {
        return res.status(400).json({ message: "Market title is required" });
      }
      const { default: Anthropic } = await import("@anthropic-ai/sdk");
      const anthropic = new Anthropic({
        apiKey: process.env.ANTHROPIC_API_KEY
      });
      const prompt = `Analyze this prediction market and provide insights in JSON format:
...
Please provide analysis in this exact JSON structure:
{
  ...
  "sentimentAnalysis": "Brief explanation of market sentiment",
  "trendPrediction": "Brief trend prediction based on the market topic",
  "trendDirection": "bullish|bearish|sideways",
  "riskAssessment": "low|medium|high",
  "riskFactors": "Key risk factors to consider",
  "recommendation": "Brief trading recommendation",
  "confidence": 0.75
}

Focus on:
1. Market sentiment based on the topic
2. Trend prediction considering current events
3. Risk assessment for this type of prediction
...
Keep responses concise and actionable.`;
      ...
      let aiResponse;
      try {
        aiResponse = JSON.parse(message.content[0].text);
      } catch (parseError) {
        aiResponse = {
          sentiment: "neutral",
          sentimentConfidence: 0.5,
          sentimentAnalysis: "Analysis temporarily unavailable",
          trendPrediction: "Market analysis in progress",
      ...
      res.status(500).json({
        ...
        error: {
          sentiment: "neutral",
          sentimentConfidence: 0.5,
          sentimentAnalysis: "AI analysis temporarily unavailable",
          trendPrediction: "Please try again later",
          trendDirection: "sideways",
          riskAssessment: "medium",
  ...
  app2.post("/api/auth/web3auth/social", async (req, res) => {
    ...
    try {
      ...
      const userEmail = email || `${provider}@web3auth.user`;
      const wallets = await createWeb3AuthWallets(provider, userEmail);
      const userData = {
        id: `web3auth_${provider}_${Date.now()}`,
        email: userEmail,
        ...
        profileImageUrl: `https://api.dicebear.com/7.x/avataaars/svg?seed=${userEmail}`,
        walletAddress: wallets.ethereum.address,
        balance: "1000"
        // Starting balance
      };
      console.log("Creating Web3Auth social user:", {
        id: userData.id,
        provider,
        ethAddress: wallets.ethereum.address,
        solAddress: wallets.solana.address
      });
      const user = await storage.upsertUser(userData);
      req.session.user = {
        claims: {
      ...
      await new Promise((resolve, reject) => {
        req.session.save((err) => {
          if (err) {
            ...
            console.log("Session saved successfully for Web3Auth user:", user.id);
            resolve();
          }
        });
      });
      console.log("Web3Auth social login successful:", {
        userId: user.id,
        provider,
      ...
      console.error("Web3Auth social login failed:", error);
      res.status(500).json({
  ...
  app2.post("/api/auth/web3auth", async (req, res) => {
    try {
      console.log("\u{1F510} Web3Auth authentication request received");
      console.log("\u{1F4E8} Request body:", JSON.stringify(req.body, null, 2));
      console.log("\u{1F36A} Session ID before auth:", req.sessionID);
      console.log("\u{1F517} Request origin:", req.headers.origin);
      ...
      if (existingUser) {
        ...
        req.session.user = {
          claims: {
            ...
            picture: existingUser.profileImageUrl
          },
          access_token: `web3auth_token_${Date.now()}`,
          expires_at: Math.floor(Date.now() / 1e3) + 3600
        };
        await new Promise((resolve, reject) => {
          req.session.save((err) => {
            if (err) {
              ...
              console.log("\u{1F36A} Session ID after save:", req.sessionID);
              console.log("\u{1F4BE} Session data after save:", req.session);
              resolve();
            }
          });
        });
        console.log("\u{1F4E4} Sending response for existing user:", existingUser.id);
  ...
  app2.post("/api/logout", (req, res) => {
    const session2 = req.session;
    console.log("Processing logout request - Session ID:", req.sessionID);
    const clearAllCookies = () => {
      const requestCookies = req.headers.cookie || "";
      const allCookieNames = requestCookies.split(";").map((cookie) => {
      ...
      const cookiesToClear = [
        ...
        "sid",
        "express.sid",
        ...allCookieNames.filter((name) => !name.includes(":"))
        // Exclude cookies with colons
      ];
      const uniqueCookies = [...new Set(cookiesToClear)];
      uniqueCookies.forEach((cookieName) => {
        if (cookieName && typeof cookieName === "string" && cookieName.trim() && cookieName.length > 0) {
          try {
            ...
            console.warn(`Failed to clear cookie '${cookieName}':`, cookieError.message);
          }
        }
      });
    };
    if (session2) {
      session2.loggedOut = true;
      session2.user = null;
      session2.save((saveErr) => {
        if (saveErr) {
        ...
        session2.destroy((err) => {
          ...
          clearAllCookies();
          console.log("User logged out - session destroyed and all cookies cleared");
          res.json({
            message: "Logged out successfully",
            sessionDestroyed: !err,
            cookiesCleared: true
          });
        });
      });
      ...
      clearAllCookies();
      console.log("User logged out - no active session, all cookies cleared");
      res.json({
        message: "Logged out successfully",
  ...
  async function getUserWalletInfo(userId) {
    ...
    const [ethData, solData] = await Promise.all([
      ethereumAddress && ethereumAddress !== "Not connected" && ethereumAddress.length === 42 ? fetchEthereumTokens(ethereumAddress).catch(() => ({ eth: "0", tokens: [] })) : Promise.resolve({ eth: "0", tokens: [] }),
      solanaAddress ? fetchSolanaTokens(solanaAddress).catch(() => ({ sol: "0", tokens: [] })) : Promise.resolve({ sol: "0", tokens: [] })
    ]);
    return {
      ethereum: {
  ...
  app2.post("/api/wallet/withdraw", isAuthenticated, async (req, res) => {
    try {
      const userId = req.user.claims.sub;
      const { amount, address, type } = req.body;
      if (!amount || !address || !type) {
        return res.status(400).json({ error: "Amount, address, and type are required" });
      }
      const withdrawAmount = parseFloat(amount);
      if (isNaN(withdrawAmount) || withdrawAmount <= 0) {
        return res.status(400).json({ error: "Invalid withdrawal amount" });
      }
      const isValidAddress = type === "eth" ? /^0x[a-fA-F0-9]{40}$/.test(address) : /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(address);
      if (!isValidAddress) {
        return res.status(400).json({ error: `Invalid ${type.toUpperCase()} address format` });
      }
      const walletInfo = await getUserWalletInfo(userId);
      const currentBalance = type === "eth" ? parseFloat(walletInfo.ethereum?.balance || "0") : parseFloat(walletInfo.solana?.balance || "0");
      if (withdrawAmount > currentBalance) {
        return res.status(400).json({ error: "Insufficient balance for withdrawal" });
      }
      let usdValue = 0;
      if (type === "eth") {
        usdValue = withdrawAmount * 3500;
      } else if (type === "sol") {
        usdValue = withdrawAmount * 140;
      }
      const user = await storage.getUser(userId);
      const currentPlatformBalance = parseFloat(user?.balance || "0");
      const newBalance = Math.max(0, currentPlatformBalance - usdValue);
      await storage.updateUserBalance(userId, newBalance.toString());
      console.log(`Withdrawal processed: ${withdrawAmount} ${type.toUpperCase()} to ${address} for user ${userId}`);
      console.log(`Platform balance updated: $${currentPlatformBalance.toFixed(2)} -> $${newBalance.toFixed(2)}`);
      res.json({
        success: true,
        message: `Successfully withdrew ${withdrawAmount} ${type.toUpperCase()} to ${address}`,
  ...
  app2.get("/api/download/namecheap-deploy", (req, res) => {
    try {
      const archivePath = path.join(process.cwd(), "bets-fun-namecheap.tar.gz");
      if (!fs.existsSync(archivePath)) {
        return res.status(404).json({ message: "Deployment package not found" });
      }
      res.setHeader("Content-Type", "application/gzip");
      res.setHeader("Content-Disposition", 'attachment; filename="bets-fun-namecheap.tar.gz"');
      const fileStream = fs.createReadStream(archivePath);
      fileStream.pipe(res);
    } catch (error) {
      console.error("Error serving deployment package:", error);
      res.status(500).json({ message: "Failed to serve deployment package" });
    }
  });
  ...
  app2.post("/api/user/claim/:betId", isAuthenticated, async (req, res) => {
    try {
      ...
      const bet = await db.select().from(bets).where(andOperator(eq3(bets.id, betId), eq3(bets.userId, userId))).limit(1);
      if (!bet.length) {
  ...
  app2.patch("/api/admin/markets/:marketId", isAdminAuthenticated, async (req, res) => {
    try {
      ...
      await db.update(markets).set({
        ...
        updatedAt: /* @__PURE__ */ new Date()
      }).where(eq3(markets.id, parseInt(marketId)));
      res.json({
  ...
  app2.delete("/api/admin/markets/:marketId", isAdminAuthenticated, async (req, res) => {
    try {
      ...
      await db.delete(bets).where(eq3(bets.marketId, id));
      await db.delete(markets).where(eq3(markets.id, id));
      res.json({
  ...
  app2.get("/api/admin/transactions", isAdminAuthenticated, async (req, res) => {
    try {
      ...
      }).from(bets).leftJoin(users, eq3(bets.userId, users.id)).leftJoin(markets, eq3(bets.marketId, markets.id)).orderBy(desc2(bets.createdAt));
  ...
  app2.get("/api/admin/user-balances", isAdminAuthenticated, async (req, res) => {
    try {
      ...
      const userBalances = await Promise.all(allUsers.map(async (user) => {
        const userBets = await db.select().from(bets).where(eq3(bets.userId, user.id));
  ...
  app2.get("/api/subscriptions", async (req, res) => {
    try {
      const { subscriptionStore: subscriptionStore2 } = await Promise.resolve().then(() => (init_subscription_store(), subscription_store_exports));
      const plans = await subscriptionStore2.getPlans();
      const bundle = await subscriptionStore2.getBundle();
      res.json({ plans, bundle });
    } catch (error) {
      console.error("Error fetching subscription plans:", error);
      res.status(500).json({ message: "Failed to fetch subscription plans" });
    }
  });
  app2.get("/api/admin/subscriptions", isAdminAuthenticated, async (req, res) => {
    try {
      const { subscriptionStore: subscriptionStore2 } = await Promise.resolve().then(() => (init_subscription_store(), subscription_store_exports));
      const plans = await subscriptionStore2.getPlans();
      const bundle = await subscriptionStore2.getBundle();
  ...
  app2.patch("/api/admin/subscriptions/:planId", isAdminAuthenticated, async (req, res) => {
    try {
      const { planId } = req.params;
      const updatedPlan = req.body;
      console.log(`Updating subscription plan ${planId}:`, updatedPlan);
      const { subscriptionStore: subscriptionStore2 } = await Promise.resolve().then(() => (init_subscription_store(), subscription_store_exports));
      const result = await subscriptionStore2.updatePlan(planId, updatedPlan);
      if (!result) {
  ...
  app2.patch("/api/admin/subscriptions/bundle", isAdminAuthenticated, async (req, res) => {
    try {
      const bundleData = req.body;
      console.log("Updating bundle settings:", bundleData);
      const { subscriptionStore: subscriptionStore2 } = await Promise.resolve().then(() => (init_subscription_store(), subscription_store_exports));
      const result = await subscriptionStore2.updateBundle(bundleData);
      res.json({
      ...
      res.status(500).json({ message: "Failed to update bundle settings" });
    }
  });
  app2.post("/api/subscriptions/purchase", isAuthenticated, async (req, res) => {
    try {
      const userId = req.user?.claims?.sub;
      ...
      let planName;
      let planPrice;
      if (planId === "ultimate-bundle") {
        const bundle = await subscriptionStore2.getBundle();
        if (!bundle.isActive) {
          return res.status(400).json({ message: "Bundle is not available" });
        }
        const plans = await subscriptionStore2.getPlans();
        const hasMarketCreator = await storage.hasActiveSubscription(userId, "market-creator");
        const hasAIInsights = await storage.hasActiveSubscription(userId, "ai-insights");
        if (hasMarketCreator && hasAIInsights) {
          return res.status(400).json({ message: "You already have access to all features" });
        }
        planName = "Ultimate Bundle";
        planPrice = bundle.price;
        const subscription = await storage.purchaseSubscription(userId, planId, planPrice.toString());
        res.json({
        ...
        const hasSubscription = await storage.hasActiveSubscription(userId, planId);
        if (hasSubscription) {
          return res.status(400).json({ message: "You already have an active subscription to this plan" });
        }
        const subscription = await storage.purchaseSubscription(userId, planId, plan.price.toString());
        res.json({
  ...
  app2.get("/api/user/subscriptions", isAuthenticated, async (req, res) => {
    try {
      const userId = req.user?.claims?.sub;
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
  ...
  app2.get("/api/user/subscriptions/:planId/status", isAuthenticated, async (req, res) => {
    try {
      const userId = req.user?.claims?.sub;
      const { planId } = req.params;
      if (!userId) {
